...
1) When accessing the proxy object, the subdocument at the currently
   ...
   the proxy object, that subdocument is returned.
...
:func:`__init__` accepts three arguments, `parent`, `key` and `seq`. `parent`
...
# If both `key` and `seq` are provided, the key is saved in the proxy object but
  ...
  but will be validated at that time.
